...
- 多级真实行情源回退：`easyquotation -> akshare -> 历史行情 -> 离线模拟`
- 自动刷新定时器
- 可选自动执行信号
- 风控层：止损、止盈、日内交易次数上限、最小交易间隔
- 持仓按最新行情自动盯市
...
### `data.realtime_provider`

- `auto`：优先使用 `easyquotation`，失败后回退到其他数据源
- 适合本地模拟盘接入尽量真实的实时行情
...
- 下单后会记录应用级订单结果和券商级委托状态
- 模拟券商会生成订单簿、成交记录和事件日志
- 可在主界面输入订单号尝试撤单
- 每次加载配置、执行信号、撤单后都会落地账户快照和持仓快照
- 记录中心新增委托中心、成交记录、账户快照、持仓快照、事件日志

## 第四步自动化

当前版本已经支持：

- 在界面中设置止损%、止盈%、日内交易次数上限、最小交易间隔秒数
- 在界面中开启自动刷新并设置刷新周期
- 可选开启自动执行信号
- 行情表可直接看到当前数据来源
...
- 增加风控规则，如止损、仓位上限、黑名单
- 增加任务调度和自动轮询
- 为 QMT 增加实时异步回报订阅、自动重连和委托状态映射完善
